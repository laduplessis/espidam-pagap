...
\begin{center}
	...
	\textbf{\LARGE Tutorial using BEAST v2.7}\\\vspace{2mm}
...
phylogenetic trees. This tutorial is written for BEAST v2.7.8 \citep{Bouckaert2014, Bouckaert2019}.
...
the files from \url{https://github.com/laduplessis/espidam-pegep/tree/main/datasets/ebov}{here}. 
...
box pops up asking what to add select \textbf{Import Alignment} and select the datatype as \textbf{nucleotide}.
...
\item
  ...
  Leave the \textbf{File Name} as \lstinline!$(filebase).log! (this means
...
\item
  Set the \textbf{File Name} to \lstinline!$(filebase).trees!.
\item
...
Select the \textbf{Tree.height} statistic in the left hand list to look
...
\textbf{Tree.height} statistic gives the marginal posterior distribution
of the age of the root of the entire tree (that is, the tMRCA; the time to the 
most recent common ancestor).

\begin{framed}
Select \textbf{Tree.height} in the bottom left hand list in Tracer and
...
Before we can create CCD trees we have to first install the \textbf{CCD} package.

\begin{framed}
  Open \textbf{BEAUti} (if not already open)

  Select \textbf{File > Manage Packages}. Select the \textbf{CCD} package in the list and press \textbf{Install/Upgrade} (Figure \ref{fig:ccdpackage}).
\end{framed}
...
For the \textbf{Target tree type} option we will pick \textbf{MAP (CCD0)}. 
Alternatively, you can also build the MCC tree. 

\begin{framed}
Change the \textbf{Target tree type} to
\textbf{MAP (CCD0)}.
\end{framed}
...
Set the \textbf{Output File} to \lstinline!EBOV_SC.CCD0.tree!.
\end{framed}
...
\begin{framed}
%TClick on the \textbf{Clock Models} tab and select \lstinline!Relaxed Clock Log Normal!
%from the dropdown box.
Click on the \textbf{Clock Models} tab and select \lstinline!Optimised Relaxed Clock!
from the dropdown box.
\end{framed}
...
\begin{framed}
  ...
  For \textbf{ORCucldMean.c:clock} (the mean rate of the lognormal distribution used for the relaxed 
  clock) we will use the same lognormal prior we used for the 
  clock rate in the strict clock analysis (\textbf{M = 1E-3, S = 0.5, Mean in Real Space}. We will leave the prior for \textbf{ORCsigma.c:clock} and
  \textbf{ORCRates.c:clock} as the default.
\end{framed}
